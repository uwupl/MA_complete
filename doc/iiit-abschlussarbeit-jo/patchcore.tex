...
In diesem ausführlichen Abschnitt werden die Architekturen ResNet 18, ResNet 34 und ResNet 50 untersucht.\ (TODO --> RN50 inkludieren?)
...
Die nächst beste Hierarchieebene ist $j=\{1\}$. Es zeigt sich die niedrigste Laufzeit in diesem Feld von lediglich $\num{7,25}\si{\milli\second}$ je Bild auf der Desktop-CPU.\
...
Dies gilt analog auch für das ResNet 34 und ResNet 50. Zwar hat das ResNet 50 für jeden Kanal die 4-fache Merkmalslänge, die Verhältnisse zwischen den Hierarchieleveln bleiben aber \
erhalten. \\
...
Wie bereits in \ref{fig:patchcoreresnet18single} zu sehen, ist $j=4$ alleine nicht geeignet. Der AUROC kann durch Hinzunahme weiterer Hierarchielevel lediglich \
...
Es gilt aber auch, dass in allen Fällen die Instanzklassifiziergungsgenauigkeit für MVTecAD leicht zunimmt. \
...
Zwar kann im Falle von $j=\{2,3\}$ eine leichte Steigerung des AUROC festgestellt werden, diese ist aber mit einer absoluten Differenz vin $\num{0,2}\%$ auf MVTEcAD sehr gering \
und geht mit einer fas Verdoppelung der Laufzeit einher. \ Für den Fall $j=\{1,2,3\}$ ist eine deutliche Verbesserung lediglich  festzustellen.\
\begin{figure}[h]
    \centering
...
% \begin{figure}[h]
%     \centering
%     \input{tikz/slidern50singleedited}
%     \caption{ResNet 50: Einzelne Hierarchielevel im Vergleich.}
%     \label{fig:patchcoreresnet50single}
% \end{figure}
% \begin{figure}[h]
%     \centering
%     \input{tikz/slidern50combiedited}
%     \caption{ResNet 50: Verschiedene Kombinationen an Hierarchieleveln im Vergleich.}
%     \label{fig:patchcoreresnet50combi}
% \end{figure}
